...
public class OfferService {
    ...
    public OfferResponse createOffer(
        ...
        if (crop.getStatus() != CropStatus.AVAILABLE) {
            throw new BadRequestException(
                    "Offers can only be made for available crops"
            );
        }

        if (request.quantity().compareTo(crop.getQuantity()) > 0) {
            throw new BadRequestException(
                    "Offer quantity cannot exceed available crop quantity"
            );
        }

        boolean existingOffer =
                offerRepository.existsByCropIdAndBuyerIdAndStatus(
                        crop.getId(),
                        buyer.getId(),
                        OfferStatus.PENDING
                );
    ...
    public List<OfferResponse> getMyReceivedOffers(Long userId) {
        ...
        return offerRepository
                .findByCropFarmerIdOrderByCreatedAtDesc(farmer.getId())
                .stream()
                .map(this::toResponse)
                .toList();
    }
    ...
    public OfferResponse acceptOffer(Long userId, Long offerId) {
        ...
        if (!crop.getFarmer().getId().equals(farmer.getId())) {
            throw new BadRequestException(
                    "You can only accept offers for your own crops"
            );
        }
        ...
        if (offer.getQuantity().compareTo(crop.getQuantity()) > 0) {
            throw new BadRequestException(
                    "Offer quantity is greater than the available crop quantity"
            );
        }
    ...
    public OfferResponse rejectOffer(Long userId, Long offerId) {
        ...
        if (!crop.getFarmer().getId().equals(farmer.getId())) {
            throw new BadRequestException(
                    "You can only reject offers for your own crops"
            );
        }
    ...
    @Transactional
    public OfferResponse counterOffer(
            Long userId,
            Long offerId,
        ...
        if (!crop.getFarmer().getId().equals(farmer.getId())) {
            throw new BadRequestException(
                    "You can only counter offers for your own crops"
            );
        }
    ...
    public OfferResponse acceptCounterOffer(Long userId, Long offerId) {
        ...
        if (offer.getStatus() != OfferStatus.COUNTERED) {
            throw new BadRequestException(
                    "Only countered offers can be accepted"
            );
        }

        if (offer.getCounterPrice() == null) {
            throw new BadRequestException(
                    "Counter price is not available"
            );
        }
